...
- [x] Post-launch — three picks from the "what else?" brainstorm: the shelf radio now reads the weather forecast aloud (weather was always a pure function of seed and time — the radio just says tomorrow out loud, next-rain-for-the-barrel included); winter snow — 'rain' periods fall as snow in all three rooms, the lawn goes white with keeper footprints, and a three-tap snowman (cap, carrot and scarf included) pays a little reward each winter before melting in spring; and plant birthdays — every plant knows its planting date, and each anniversary brings confetti over the pot, a diary page with the age, and a small present (caught up gently after time away)
- [x] Post-launch — the rest of the brainstorm, in one go: a wishable rainbow when sunshine follows rain; the apple tree living by the calendar (blossom, green apples, three pickable autumn reds that ARE the day's luck jar); windy periods that set the clothesline flapping, tumble leaves across the lawn — and once an autumn blow in a volunteer seedling worth potting up; honest post in the letterbox (flag up only when the postman came, eight letters from relatives like Aunt Drosera, small treat enclosed); a buyable garden pond with cattails, a summer lily and a greetable dragonfly; a keeper wardrobe (straw hat for sun, yellow raincoat and boots for rain — boots unlock puddle stomping); propagation — a thriving grown plant donates a leaf pulling that roots into a free clone, cultivar included; seed gifts riding inside ordinary chat messages (the server stays exactly as dumb as ever — both sims keep the books); and a December advent calendar with 24 doors where catching up is half the tradition
