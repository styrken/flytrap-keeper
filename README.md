...
- [x] Post-launch — full-circle camera: orbit all the way around the keeper in every room — solid backdrops (the window wall, the house facade) duck out of sight while the camera is behind them, and the outdoor sky is a drum around the whole lawn instead of a single north-side backdrop
